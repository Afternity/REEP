...
namespace REEP.Application.Features.UserFeatures.UserTypeFeatures.UserTypes.Queries.GetUserTypeList
{
        ...
        : IRequestHandler<GetUserTypeListQuery, UserTypeListVm>
    {
        ...
        public GetUserTypeListQueryHandler(
            ...
            IMapper mapper,
            ILogger<GetUserTypeListQueryHandler> logger) =>
        ...
        {
            var entities = await _context.UserTypes
                .Where(userTypes => userTypes.IsDeleted == request.IsDeleted)
                .ProjectTo<UserTypeLookupDto>(_mapper.ConfigurationProvider)
                .ToListAsync(cancellationToken);

            return new UserTypeListVm { UserTypes = entities };
        }
    }
}
